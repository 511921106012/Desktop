...
pevent_t * pstate_sub_preprocessor_main_handler(FILE *fd, int ch)
{
	...
	if((ch >= 'a' && ch <= 'z') && word_idx < WORD_BUFF_SIZE - 1 && event_data_idx < PEVENT_DATA_SIZE - 1)
	{
		word[word_idx++] = ch;
		pevent_data.data[event_data_idx++] = ch;
		// printf("%c -> ch    ",ch);
	...
	else
	{
		...
		else if((strcmp(word,"define") == 0) || (strcmp(word,"ifdef") == 0) || (strcmp(word,"endif") == 0))
		{
		...
		else /* unrecognized directive (#pragma, #ifndef, #undef, #else, #elif, ...) - close it here instead of leaving stale state */
		{
			state_sub = PSTATE_SUB_PREPROCESSOR_MAIN;
			set_parser_event(PSTATE_PREPROCESSOR_DIRECTIVE,PEVENT_PREPROCESSOR_DIRECTIVE);
		}
...
pevent_t * pstate_header_file_handler(FILE *fd, int ch)
{
	...
	{
		fseek(fd, -1, SEEK_CUR);
		set_parser_event(PSTATE_HEADER_FILE, PEVENT_REGULAR_EXP);
		return &pevent_data;
	}
	if(ch == '<')
	{
		
		while((ch = fgetc(fd)) != '>' && ch != EOF && event_data_idx < PEVENT_DATA_SIZE - 1 && word_idx < WORD_BUFF_SIZE - 1)
		{
			word[word_idx++] = ch;
			pevent_data.data[event_data_idx++] = ch;
		}
	...
	else if(ch == '"')
	{
		...
		do
		{
			prev = ch;
			ch = fgetc(fd);
			if(ch == EOF || event_data_idx >= PEVENT_DATA_SIZE - 1 || word_idx >= WORD_BUFF_SIZE - 1)
				break;
			word[word_idx++] = ch;
			pevent_data.data[event_data_idx++] = ch;
		}
...
pevent_t * pstate_reserve_keyword_handler(FILE *fd, int ch)
{
	...
	if(((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || (ch == '_'))
		&& word_idx < WORD_BUFF_SIZE - 1 && event_data_idx < PEVENT_DATA_SIZE - 1)
	{
		word[word_idx++] = ch;
		pevent_data.data[event_data_idx++] = ch;
	}
	else
	{
		word[word_idx] = '\0';
		fseek(fd,-1,SEEK_CUR);
		int prop = is_reserved_keyword(word);

		if(prop == RES_KEYWORD_DATA || prop == RES_KEYWORD_NON_DATA)
		{
			pevent_data.property = prop; 
...
pevent_t * pstate_numeric_constant_handler(FILE *fd, int ch)
{
	 ...
	 * else return NULL
	 */
	if(((ch >= '0' && ch <= '9') || (ch == 'x') || (ch == 'X') || (ch == '.') || (ch >= 'a' && ch <= 'f') || (ch >= 'A' && ch <= 'F'))
		&& event_data_idx < PEVENT_DATA_SIZE - 1)
	{
		pevent_data.data[event_data_idx++] = ch;
	}
	else
...
pevent_t * pstate_string_handler(FILE *fd, int ch)
{
	...
	if(ch == '"' && !prev_was_backslash) /* real closing quote, not an escaped \" */
	{
	...
	prev_was_backslash = (ch == '\\') ? !prev_was_backslash : 0;
